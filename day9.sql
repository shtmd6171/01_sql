...
-- b) 두 가지 기준으로 그룹화 적용
SELECT NVL(TO_CHAR(e.hiredate,'YYYY'),'해당 없음') "입사 년도"
      ,NVL(TO_CHAR(e.hiredate,'MM'),'해당 없음')   "입사 월"
      ,COUNT(*)                                    "인원(명)"
FROM emp e
...
-- c) 년도별, 월별 입사 인원의 출력 가로 표 형태로 변환
-- : 년도 추출 : TO_CHAR(e.hiredate, 'YYYY')
--     월 추출 : TO_CHAR(e.hiredate, 'MM')

-- : e.hiredate에서 추출한 월의 값이 01이 나올 때
...
SELECT NVL(TO_CHAR(e.hiredate,'YYYY'),'해당 없음') "입사 년도"
  ...
  FROM emp e
 GROUP BY NVL(TO_CHAR(e.hiredate,'YYYY'),'해당 없음')
 ORDER BY "입사 년도"
;
