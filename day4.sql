...
-- 이름의 둘 째 자리부터 LA가 들어가는 이름을 가진 직원의
-- 사번, 이름 조회
SELECT e.empno
      ,e.ename
  FROM emp e
...
SELECT e.empno
 ...
 ;
 -- ==> 위처럼 like 조건을 작성하면 j 뒤에 적어도 한 글자가 있는
...
-- NULL값을 가진 컬럼은 비교 연산자(=, != <>)와 연산이 불가능하므로
-- NULL값 비교를 위한 연산자가 따로 존재함에 주의!

-- col = null ==> null에 대해서는 = 동일비교 연산자를 사용 못함
-- col != null 와
-- col <> null ==> 에 대해서는 다름을 비교하는 연산자를 사용 못함

-- 27) 상사(mgr)가 지정되지 않은 직원의 사번, 이름, 상사 사번(mgr) 조회
SELECT e.empno
      ,e.ename
      ,e.mgr
  FROM emp e
 WHERE e.mgr = NULL -- null 데이터는 비교 연산자
                    -- 연산할 수 없는 값
;
-- 인출된 모든 행: 0
...
SELECT '급여가 10000이 넘는 직원이 존재하지않음' AS "시스템 메세지"
FROM dual -- 1행만 데이터가 있는 공용 테이블
...
SELECT '안녕하세요' || ' 오라클에서 배우고 있어요!' AS greeting
FROM dual
;
--안녕하세요 오라클에서 배우고 있어요!

--dual 테이블 활용, 오늘의 날씨를 알려주는 문장
...
-----------------------------------------------
-- (6) 연산자 6. 집합연산자
-- 첫 번째 쿼리 : 부서테이블의 모든 정보 조회 => 4행 데이터
SELECT d.deptno
      ,d.dname
      ,d.loc
  FROM dept d
 UNION ALL
-- 두 번째 쿼리 : 부서번호가 10인 부서의 정보 조회 => 1행 데이터
SELECT d.deptno
      ,d.dname
      ,d.loc
FROM dept d
...
-- 주의! : 두 쿼리의 조회 결과의 컬럼의 갯수, 데이터 타입의 순서가 일치해야 함
-- 1) 오류상황 : 첫 쿼리의 컬럼 수 :  3
--               둘 째 쿼리 컬럼 수 : 2
SELECT d.deptno
      ,d.dname
